...
impl<'a> ApplicationHandler for App<'a> {
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
        let new_title = self.title.to_owned() + self.colormap_name;
        let window_attributes = Window::default_attributes().with_title(new_title);

        let window = Arc::new(event_loop.create_window(window_attributes).unwrap());

        self.state = Some(pollster::block_on(async {
            State::new(window, self.vertex_data, self.light_data).await
        }));
